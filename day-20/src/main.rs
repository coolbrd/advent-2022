...
fn main() {
    ...
    let sequence = lines.iter().map(|line| Box::new(line.parse::<i64>().unwrap())).collect::<Vec<Box<i64>>>();
    let mut new_sequence = sequence.iter().collect::<Vec<&Box<i64>>>();
    mix_sequence(&sequence, &mut new_sequence);
    let answer = calculate_sequence_answer(&new_sequence);
    println!("Answer: {}", answer);

    let sequence = sequence.iter().map(|x| Box::new(**x * DECRYPTION_KEY)).collect::<Vec<Box<i64>>>();
    let mut new_sequence = sequence.iter().collect::<Vec<&Box<i64>>>();
    for _ in 0..10 {
        mix_sequence(&sequence, &mut new_sequence);
    }
...
fn mix_sequence<'a>(original_sequence: &'a Vec<Box<i64>>, new_sequence: &mut Vec<&'a Box<i64>>) {
    ...
    while i < len {
        ...
        let num_position_in_new_sequence = new_sequence.iter().position(|x| *x as *const _ == num).unwrap();
        let new_position = (num_position_in_new_sequence as i64 + **num) % (len - 1);
        let new_position = if new_position <= 0 { (len - 1) + new_position } else { new_position };
...
fn calculate_sequence_answer(sequence: &Vec<&Box<i64>>) -> i64 {
    ...
    let answer = [1000, 2000, 3000].iter().map(|offset| {
        (zero_pos + offset) % sequence.len() as usize
    }).map(|pos| **sequence[pos]).sum::<i64>();
    answer
}
